...
public class MergeActionJson extends AnAction {

    private static final Logger log = Logger.getInstance(MergeActionJson.class);
                ...
                if (Files.exists(Paths.get(path))) {
                    ...
                    switch (spec) {
                        case OAS2:
                            ...
                            io.swagger.util.Json.pretty().writeValue(new File(fH.mergeName(path, JSON)), oas2);
                            VirtualFileManager.getInstance().syncRefresh();
                            break;
                        case OAS3:
                            OpenAPI oas3 = new OpenAPIV3Parser().read(path);
                            io.swagger.v3.core.util.Json.pretty().writeValue(new File(fH.mergeName(path, JSON)), oas3);
                            VirtualFileManager.getInstance().syncRefresh();
                            break;
                        default:
